...
if ($nb_emails >0) {
    $messages[]= "un user est deja inscrit avec cet email. Peut-être avez -vous deja un compte?";
}
// Fixtures
// Etape 5 > Vérifier que le login est unique (pas déjà dans la DB)
$res_login = findOneUserBy('login',$datas_form['login']);
$nb_logins = count($resultat);
if ($nb_logins >0) {
...
if (strlen($datas_form['password'])<8) {
    $messages[] = "Ce mot de passe doit avoir au moins 8 caractères?";
}
// Etape 7 > Si tout est ok > insertion des datas dans la DB > redirection vers page confirmation inscription
if(count($messages) == 0) {
    // Exécuter une requête SQL pour transferer les données dans le form dans la base de données
    addUser($datas_form);
    // Démarrage de session
    session_start ();
    // on stcke dans la session une donnée "peuve"

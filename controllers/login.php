...
// 5- Commparer le mot de passe stocké dans la bd au mot de passe
//// saisi par le user
...
//    die();
    ...
    if($user[0] ['is_admin'] == false) {
        $messages[]= "Vous n'avez pas le droit d'acceder";
    } else{
        // 7- Si user est admin > démarrage session, stockage dans la session
...
// on cumule les messages d'érreur et on redirige le user sur le form de login avec affichage de toutes ses erreurs
session_start();
